...
class CTNet(nn.Module):
    def __init__(self, batch, in_channels, out_channels):
        ...
        self.help_fc1 = nn.Linear(66, 32)
        self.help_fc2 = nn.Linear(32, 16)

        self.fc2 = nn.Linear(self.in_channels-1, self.in_channels-1)
        self.conv2 = nn.Conv1d(16, 32, kernel_size=(3,), padding=(1,), stride=(1,))
    ...
    def forward(self, x):
        ...
        x = self.mp1(x)
        x = self.bn1(x)
        x = self.help_fc1(out) + x
        x = self.fc2(x)
        x = self.conv2(x)
        out2 = copy(x)
        x = self.mp2(x)
        x = self.bn2(x)
        x += self.help_fc2(out2)
        x = self.pt(x)
        x = self.fc3(x)
        x = self.act(x)
        return x
...
if __name__ == '__main__':
    ...
    model = CTNet(batch, in_features, out_features)
