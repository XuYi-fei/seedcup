...
if __name__ == '__main__':
    ...
    model = ResNet(ResidualBlock, [2, 2, 2], in_features)
    model = model.to(device)
    ...
    train_dataset = SeedDataset("./data/unmodified/train.csv")
    train_dataloader = DataLoader(
        train_dataset, batch_size=batch_size, shuffle=True)

    valid_dataset = SeedDataset("./data/unmodified/valid.csv")
    ...
    for t in range(epochs):
        ...
        if(os.path.isdir(f"./checkpoints/resnet") == 0):
            os.mkdir(f"./checkpoints/resnet")
        torch.save(model.state_dict(), f"./checkpoints/resnet/{t}_epoc.pt")
